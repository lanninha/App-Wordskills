...
class Login : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        botaoLogin.setOnClickListener {
            ...
            if (username.text.toString().isEmpty() || password.text.toString().isEmpty()) {
                Toast.makeText(this, "Insira login e senha primeiramente!", Toast.LENGTH_SHORT).show()
            } else {
    ...
    // Destaca campos com cor de erro
    private fun highlightFields() {
        val corError = ContextCompat.getColor(this, R.color.vermelho)
        val corOriginal = ContextCompat.getColor(this, R.color.roxoEscuro)

        // Cria uma função para aplicar a cor ao fundo
        fun applyBackgroundTint(editText: EditText, color: Int) {
            val drawable = editText.background as GradientDrawable
            drawable.setColor(color)
        }

        // Aplica a cor de erro
        applyBackgroundTint(username, corError)
        applyBackgroundTint(password, corError)

        // Reverte a cor após um atraso de 2 segundos
        Handler(Looper.getMainLooper()).postDelayed({
            applyBackgroundTint(username, corOriginal)
            applyBackgroundTint(password, corOriginal)
